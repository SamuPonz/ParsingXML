...
//SCRIVE IL FILE FILTRANDO SOLO I service_id PRESENTI IN CALENDAR DATES
    public boolean write(String filename, Trip viaggio, Route tratta, StopTimes fermata, Calendar_Dates calendario) {
		System.out.println("Scrittura su file");
		XMLOutputFactory output = XMLOutputFactory.newInstance();
		XMLStreamWriter writer;
		int j;
		try {
			...
			writer.writeStartElement("services");
			for (int i = 0; i < viaggio.getService_id().size(); i++){
				if (calendario.trovaID(viaggio.getService_id().get(i))) {
					writer.writeStartElement("serivice");
					writer.writeAttribute("id", viaggio.getService_id().get(i));
					writer.writeStartElement("trip");
					String id = viaggio.getTrip_id().get(i);
